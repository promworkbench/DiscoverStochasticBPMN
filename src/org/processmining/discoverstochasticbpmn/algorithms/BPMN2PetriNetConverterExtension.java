...
public class BPMN2PetriNetConverterExtension extends BPMN2PetriNetConverter {
    public BPMN2PetriNetConverterExtension(BPMNDiagram bpmn, BPMN2PetriNetConverter_Configuration config) {
        super(bpmn, config);
    }
    ...
    private void translateEdges() {
        for (BPMNNode n : bpmn.getNodes()) {
            ...
            if (n instanceof Gateway)
                ...
                if ( ((Event)n).getEventType() == Event.EventType.END ) {
    ...
    private void translateEdges(Collection<Flow> flows, boolean mergeToSinglePlace) {
        ...
        if (mergeToSinglePlace) {
            ...
            for (Flow f : flows) {
                ...
                Place p = net.addPlace(p_label);
                flowMap.put(f, p);
//                System.out.println("Added the element, " + p.getLabel() + " to flowMap" + flowMap.toString());
     ...
     * place through translateEdges().
     *
     * @param n the BPMNNode corresponding to the current transition t
     * @param t the current transition
     */
     ...
     *
     * @param a the current activity being translated
    ...
    /**
     * @param a the current atomic activity being translated
    ...
    private boolean translateActivityAtomic(Activity a) {
        ...
        if (!getBoundaryEvents(a).isEmpty()) return false;
        return !(a instanceof SubProcess) || !config.linkSubProcessToActivity;
    }
    ...
    private void translateXORGateway(Gateway g) {
        ...
        gatewayMap.put(g, new XORChoiceMap());
...
//                System.out.println("Added the element, " + t.getLabel() + " to choiceMap...");
     ...
     * @param nodeMap map BPMN node with corresponding petri net places and transitions
     * @param n the current BPMN node
